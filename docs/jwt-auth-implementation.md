...
All other existing endpoints now require a valid JWT in the `Authorization` header.
...
```java
@GetMapping("/my-lists")
public List<ShoppingListResponse> getMyLists() {
    String userId = (String) SecurityContextHolder
            .getContext()
            .getAuthentication()
            .getPrincipal();

    return shoppingListService.getListsByOwner(UUID.fromString(userId));
}
```
